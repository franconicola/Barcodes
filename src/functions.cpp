...
void binarization(const Mat & input, cv::Mat & output){

	Mat binarization_image = Mat::zeros(input.size(), input.type());
	Mat kernel(100, 3, CV_8U, Scalar(1));

	threshold(input, binarization_image, 0, 255, CV_THRESH_BINARY_INV | CV_THRESH_OTSU);
...
void orientation(Mat & input_labeling, Mat & input_source, Mat & output, int & index, vector<vector<float>> & parameters){
	...
	// Find contours
	findContours(input_labeling, contours, hierarchy, CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE, Point(0,0) );
	...
	int outMargin = 30;
		...
		for (int i = 0; i < (int) scanProfile.size(); i++) {
			if (scanProfile.at(i) > 0){
				...
				rectangle(image_out, pt1, pt2, Scalar(255 - i, i / 3, i), CV_FILLED);
			}
		}
		//add notes
		string left = "0";
